...
function AubUtilProcessChatText(sourceent,text) --process dialog 'n text players and sap bots post into chat, along with any other source, sourceent will be nil if not provided
    if (SERVER) then
        if (sourceent != nil && IsValid(sourceent)) then
            table.insert(AubUtil_ChatLogAll,"["..GetBestName(sourceent).."]:"..text)
        else
            table.insert(AubUtil_ChatLogAll,"[Name Not Provided]:"..text)
        end
    end
end
hook.Add( "PlayerSay", "AubUtilSayGrab", function( ply, text )
	AubUtilProcessChatText(ply,text)
end)
hook.Add( "PrintMessage", "AubUtilAnySayGrab", function( ttype, text )
    if (ttype == HUD_PRINTTALK) then
	    AubUtilProcessChatText(nil,text)
    end
end)

function GetBestName(entity)
    return(GetBestName(entity,false))
end
function GetBestName(entity,noFormatting) --gets the best readable name of an entity, depending on what it is, and how its name is formatted
    ...
    if (entity != NULL and entity != nil) then
        ...
        print("Aub Util Unable To Get Entity name from a NULL Entity")
        return(nil)
    end
end
...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
-- -- -- -- Aub Utility LLM area, aka the Groq functionality -- -- -- -- --
-- -- -- -- -- -- for other addons besides S.A.P Bots -- -- -- -- -- -- -- 

if (AUBUTIL_LLMKEY == nil) then AUBUTIL_LLMKEY = "" end --llm key global value
AUBUTIL_LLMCOOLDOWN = 0 --the cooldown value, a CurTime() in the future
AUBUTIL_LLMMODEL = "llama-3.1-8b-instant"

if SERVER then
    concommand.Add("aubutil_llm_setkey", function(ply, cmd, args)
        if not ply:IsAdmin() then
            print([[-= You must be Admin to set the Aub Utility LLM Groq API key or interact with it. =-]])
            return
        end
        if args[1] then
            if (args[1] == "" || args[1] == nil || args[1] == " ") then
                print([[-= Aub Utility LLM Groq API key (]]..args[1]..[[) unable to process,
                visit (https://console.groq.com/keys) to get your Groq key,
                it is seemingly fully free and has less limits than Chat GPT,
                hence why this uses Groq. =-]])
            else
                AUBUTIL_LLMKEY = args[1]
                print([[-= Aub Utility LLM Groq API key set to:]]..args[1]..[[ =-]])
            end
        end
    end, nil, "Run with a Groq API key as its first argument, to set the API key for Aub Utility LLM functions")
end
...
local timeoutVal = nil --temp value
function AubUtilLLM_ProcessOutPrompt(tableIn) --processes the out prompt of the llm into errors or formatted outputs, to then be used more as string
    if (tableIn["error"] != nil) then
        local timeoutVal = getTimeout(tableIn["error"]["message"])
        if (timeoutVal != nil) then
            AUBUTIL_LLMCOOLDOWN = (CurTime() + 0.1 + getTimeout(tableIn["error"]["message"]))
        end
        return("error: "..tableIn["error"]["type"].." "..tableIn["error"]["message"])
    end
    if (tableIn["choices"] == nil) then
        return("error no choices for Aub Utility LLM prompt out")
    end
...
local llmtemp = nil --temp value
function AubUtilLLM_SendPrompt(debugName,prompt,functionOut)
    AubUtilLLM_SendPrompt(debugName,prompt,functionOut,0.5,64,nil)
end
function AubUtilLLM_SendPrompt(debugName,prompt,functionOut,heat)
    AubUtilLLM_SendPrompt(debugName,prompt,functionOut,heat,64,nil)
end
function AubUtilLLM_SendPrompt(debugName,prompt,functionOut,heat,maxTokens,entInclude)
    if (CLIENT) then return end --must be ran on server, server-side only function
    if (AUBUTIL_LLMKEY == "" || AUBUTIL_LLMKEY == nil || AUBUTIL_LLMKEY == " ") then --must have key be set
        print([[-= Aub Utility LLM Groq API key (]]..AUBUTIL_LLMKEY..[[) unable to process,
                visit (https://console.groq.com/keys) to get your Groq key,
                it is seemingly fully free and has less limits than Chat GPT,
                hence why this uses Groq. =-]])
        print([[-= Use the console command of "aubutil_llm_setkey KEY" with your key, while admin,
        to set your key for the Aub Utility addon's LLM features to function =-]])
        return
    end
    if (AUBUTIL_LLMCOOLDOWN <= CurTime()) then
        HTTP( {
            failed = function(reason)
                print("Aub Utility LLM HTTP post failed, try running the version of gmod that is good with HTTP stuff if you are not already.", reason)
            end,
            success = function(code, body, headers)
                if (functionOut != nil) then
                    llmtemp = AubUtilLLM_ProcessOutPrompt(util.JSONToTable(body)) --non-error (usually) out
                    if (AUBUTIL_LLMCOOLDOWN > CurTime()) then
                        print("Aub Utility LLM api cannot post, as cooldown is active for "..(AUBUTIL_LLMCOOLDOWN - CurTime()).." seconds.")
                    else --the output area that runs the function with the out prompt.
                        llmtemp = llmtemp:gsub('"', '') --simply remove quotes from output

                        --sapAiNetOpinionChange(llmtemp:match("%(([^)]+)%)"),sapEntity)
                        --llmtemp = sapProcessPromptExtras(llmtemp:gsub("%b()", ""),sapEntity)
                        --print("to say : "..tosayTemp)
                        if (entInclude == nil) then
                            functionOut(llmtemp)
                        else
                            functionOut(llmtemp,entInclude)
                        end
                    end
                else
                    print("Aub Utility LLM api got, but function to output via is nil?")
                end
            end,
            method = "POST",
            headers = {
                ["Authorization"] = "Bearer " .. AUBUTIL_LLMKEY,
                ["Content-Type"] = "application/json"
            },
            body = numsInStringToInt(util.TableToJSON({
                ["model"] = AUBUTIL_LLMMODEL,
                ["messages"] = {{
                    ["role"] = "system",
                    ["content"] = prompt
                }},
        ...
        print("Aub Utility LLM api cannot send prompt, due to cooldown active of "..(AUBUTIL_LLMCOOLDOWN - CurTime()).." seconds.")
    end
end
...
function AubUtilGetEntityByName(name) --very expensive, do not use this usually!!!!
    for k, v in pairs(ents.GetAll()) do
        if GetBestName(v,true):lower() == name:lower() then
            return v
        end
    end
    return nil
...
function AubUtilKillAnything(ent)
    if (IsValid(ent) && ent != nil) then
        ent:TakeDamage(9999999999,nil,nil)
    end
end

function AubUtilObliterate(ent) --kick any entity from reality smh, with a explosion
    if SERVER then
...
function AubUtilSpawnRandomPropAt(pos)
    AubUtilDeclarePropDefaultData()
    local prop = ents.Create('prop_physics')
    prop:SetModel(AubUtil_RandomPropsList[math.random(#AubUtil_RandomPropsList)])
    prop.IsSapProp = true
...
function AubUtilDeclarePropDefaultData() --kinda like what sap bots use, but for every addon, without sap bots needed
    if (AubUtil_RandomPropsList == nil) then
        AubUtil_RandomPropsList = {
